...
class UIDReferenceField(StringField):
    ...
    def get_object(self, context, value):
        ...
        if not value:
            ...
            try:
                # we must try using acquisition to get the catalog,
                # sometimes get_tool fails to find the catalog here.
                uc = getToolByName(context, 'uid_catalog')
    ...
    def set(self, context, value, **kwargs):
        ...
        if self.multiValued:
            ...
            for val in value:
                if val not in ret:
                    ret.append(self.to_uid(context, val))
        else:
